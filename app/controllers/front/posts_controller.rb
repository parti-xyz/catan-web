class Front::PostsController < Front::BaseController
  def show
    @current_post = Post.includes(:survey, :current_user_post_reader, :current_user_bookmark, :current_user_upvotes, :last_stroked_user, :file_sources, :label, issue: [:group], announcement: [ current_user_audience: [ :member ] ], user: [ :current_group_member ], comments: [ :wiki_history, :file_sources, :current_user_bookmark, :current_user_upvotes, user: [ :current_group_member ] ], wiki: [ :last_wiki_history ], poll: [ :current_user_voting ])
      .find(params[:id])
