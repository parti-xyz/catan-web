...
FROM `TABLES` where table_schema like "catan_web" group by table_schema;
SELECT concat("ALTER TABLE `",table_schema,"`.`",table_name,"` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;") as _sql
FROM `TABLES` where table_schema like "catan_web" group by table_schema, table_name;
...
FROM `COLUMNS` where table_schema like "catan_web" and data_type in ('varchar');
...
FROM `COLUMNS` where table_schema like "catan_web" and data_type in ('text','tinytext','mediumtext','longtext');
