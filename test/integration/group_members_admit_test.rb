...
class GroupMembersAdmitTest < ActionDispatch::IntegrationTest
  test '초대해요' do
    ...
    post admit_group_members_path, params: { recipients: "#{users(:one).email}, #{users(:two).nickname}, not_exists@email.com" , message: 'ok' }
  ...
  test '두번 초대는 안해요' do
    ...
    host! "private.example.com"
    post admit_group_members_path, params: { recipients: "not_exists@email.com" , message: 'ok' }
    post admit_group_members_path, params: { recipients: "not_exists@email.com" , message: 'ok2' }
  ...
  test '가입된 사용자는 초대안해요' do
    ...
    host! "private.example.com"
    post admit_group_members_path, params: { recipients: "#{users(:three).email}" , message: 'ok' }
  ...
  test '가입요청한 사용자는 초대하면 가입되어요' do
    ...
    host! "private.example.com"
    post admit_group_members_path, params: { recipients: "#{users(:member_request_to_private_group).email}" , message: 'ok' }
  ...
  test '동일한 이메일의 사용자가 있으면 초대가 안되어요' do
    sign_in(users(:admin))
    host! "private.example.com"
    post admit_group_members_path, params: { recipients: "ambiguous@test.com" , message: 'ok' }
  ...
  test '닉네임에 해당되는 사용자가 없으면 초대가 안되어요' do
    sign_in(users(:admin))
    host! "private.example.com"
    post admit_group_members_path, params: { recipients: "not_found_nickname" , message: 'ok' }
    assert groups(:private_group).invitations.empty?
  end
end
